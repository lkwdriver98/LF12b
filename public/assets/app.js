(function(){
  ...
  const STORAGE = 'tf_user';     // {id,name,email,role}
  const TOKEN_KEY = 'tf_token';  // JWT
  const API = "";                // gleicher Origin (Render)

  function getUser(){ try{ return JSON.parse(localStorage.getItem(STORAGE)||'null'); }catch{ return null; } }
  ...
  // ---- Modal-HTML bei Bedarf automatisch einfügen ----
  function ensureAuthModal(){
    if ($('#authModal')) return; // existiert schon
      ...
      <div id="authModal" class="modal hidden" role="dialog" aria-modal="true" aria-labelledby="authTitle">
        <div class="modal__panel">
          ...
          <div style="display:flex;justify-content:flex-end;margin-top:.5rem">
            <button id="authClose" class="btn" type="button">Schließen</button>
          </div>
        </div>
      </div>
    `);
  ...
  function init(){
    // sicherstellen, dass Modal vorhanden ist
    ensureAuthModal();
    ...
    // Buttons
    if (loginBtn) {
      loginBtn.onclick = (e)=>{ e.preventDefault(); open(); showLogin(); };
    }
    if (logoutBtn) {
      logoutBtn.onclick = (e)=>{ e.preventDefault(); localStorage.removeItem(TOKEN_KEY); clearUser(); applyAuth(); };
    }
    ...
    loginForm?.addEventListener('submit', async (e)=>{
      ...
      try{
        const data = await fetchJSON(`${API}/api/login`, { method:'POST', body: JSON.stringify({ email, password }) });
    ...
    registerForm?.addEventListener('submit', async (e)=>{
      ...
      try{
        const data = await fetchJSON(`${API}/api/register`, { method:'POST', body: JSON.stringify({ name, email, password }) });
        localStorage.setItem(TOKEN_KEY, data.token);
        setUser(data.user);
        applyAuth();
        close();
  ...
  // Script sicher erst nach DOM initialisieren
